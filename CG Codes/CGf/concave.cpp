...
// Function to implement the scan line polygon fill algorithm for concave polygons
void scanLineFillConcave(const vector<Point> &vertices, int fill_color[])
{
    ...
    for (int y = minY; y <= maxY; y++)
    {
        ...
        for (size_t i = 0; i < vertices.size(); i++)
        {
            ...
            if (((y >= vertices[i].y && y < vertices[next].y) ||
                ...
                ((vertices[next].y - vertices[i].y) * (vertices[i].x - vertices[prev].x) !=
                 (vertices[i].y - vertices[prev].y) * (vertices[next].x - vertices[i].x)))
            {
...
void display()
{
    ...
    // Draw concave polygon and fill it
    ...
    scanLineFillConcave(vertices, fill_color);
...
int main(int argc, char **argv)
{
    ...
    glutCreateWindow("Concave Polygon Fill Algorithm");
